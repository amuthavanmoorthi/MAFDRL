...
def _map_actions(env, acts):
    ...
    This is shared by train() and evaluation.
    """
    # p in [0, p_max]
...
def train(seed=0, U=3, Mt=2, Nr=4,
          steps=5000, batch=64, local_iters=500, fed_rounds=50):
    """
    ...
    Produces:
      ...
      - fig2_eval.png            : deterministic evaluation curve (for paper)
    """
    ...
    for r in range(fed_rounds):
        ...
        for c, (env, algo, buf) in enumerate(zip(envs, agents, buffers)):
            ...
            for t in range(local_iters):
                # Reduced exploration noise for more stable learning
                ...
                if (t + 1) % 100 == 0:
                    ...
                    print(f"[client {c}] t={t+1:04d} | mean_rew={mean_rew:.3f} | "
        ...
        # Round-level reward summary (raw training metric)
        ...
        # Use a separate seed offset so eval env is consistent across rounds
        eval_seed = seed + 1000
        eval_rew = evaluate_policy(U, Mt, Nr, eval_seed, agents[0],
...
if __name__ == "__main__":
    ...
    train(local_iters=500, fed_rounds=50)

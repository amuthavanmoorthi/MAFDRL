...
class MADDPG:
    ...
    def update(self, batch, p_bounds):
        ...
        with th.no_grad():
            ...
            q_next_in = th.cat([nobs_t.reshape(B, -1), nact.reshape(B, -1)], dim=1)
            q_next = self.critic_t(q_next_in).squeeze(-1)
            y = (rew_t.sum(dim=1) + (1.0 - done_t.max(dim=1).values) * self.gamma * q_next).detach()
        ...
        for u in range(self.U):
            ...
            a_all = act_t.clone()
            a_all[:,u,:] = a_pred
            q_in_pi = th.cat([obs_t.reshape(B,-1), a_all.reshape(B,-1)], dim=1)

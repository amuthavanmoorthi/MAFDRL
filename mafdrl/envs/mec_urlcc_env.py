...
class MECURLLCEnv:
    ...
    Core equations implemented:
      ...
      - Reward:   R_u - α p_u - β (T_u^{E2E}/T_deadline)
                  - η·(q_u / (3·L_mean)) - λ·1{T_u^{E2E} > T_deadline}
    """
    ...
    def __init__(
        ...
        alpha_energy=1e-3,        # α
        beta_latency=5e-2,        # β (acts on normalized latency)
        lambda_pen=5e-2,          # λ (deadline violation penalty)
        queue_penalty=2e-2,       # η (queue/backlog penalty)
        seed=0,
    ):
        # basic params
    ...
    def _finite_blocklength_rate(self, sinr):
        """
        Eq. (fb_rate): R ≈ log2(1+γ) - sqrt(V(γ)/D) * Q^{-1}(ε),
        with V(γ) = 1 - (1+γ)^(-2).
        """
    ...
    def reset(self):
        ...
        self.queue_bits += (
            self.rng.poisson(1.0, size=self.U) * self.L_mean * 0.2
        )
        return self._obs()
    ...
    def step(self, actions):
        ...
        g = h_eff / np.maximum(
            np.linalg.norm(h_eff, axis=1, keepdims=True), 1e-12
        )
        ...
        for u in range(self.U):
            num = np.abs(
                np.vdot(g[u], (H[u] @ f[u][:, None]).squeeze(-1))
            ) ** 2
            inter = 0.0
            for j in range(self.U):
                if j == u:
                    continue
                inter += np.abs(
                    np.vdot(g[u], (H[j] @ f[j][:, None]).squeeze(-1))
                ) ** 2
        ...
        new_bits = (
            self.rng.poisson(lam=1.0, size=self.U) * self.L_mean * 0.5
        )
        ...
        # ---------- Reward ----------
        # Normalize latency w.r.t. deadline
        T_norm = T_e2e / max(self.T_deadline, 1e-6)

        # Queue backlog penalty (encourages offloading when queues grow)
        q_norm = self.queue_bits / (self.L_mean * 3.0)

        # Base reward:
        #   R - α p - β T_norm - η q_norm - λ 1{T_e2E > T_deadline}
        reward = (
            R
            - self.alpha_energy * p
